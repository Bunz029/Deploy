...
class RoomController extends Controller
{
    ...
    /**
     * Get ALL rooms for ALL buildings in one batch call (for performance optimization)
     * This replaces 50+ individual API calls with 1 batch call
     */
    public function getAllRoomsBatch(): JsonResponse
    {
        try {
            // Only return rooms for the currently active map
            ...
            // Get all published buildings for the active map
            ...
            // Get all published rooms for all buildings at once
            $rooms = Room::whereIn('building_id', $buildingIds)
                        ->where(function($query) {
            ...
            // Transform rooms to use published data (same logic as individual endpoint)
            $publishedRooms = collect();
            foreach ($rooms as $room) {
                try {
                    $snapshot = $room->published_data;
                    ...
                    \Log::warning('Batch rooms transform error', [
                        'room_id' => $room->id ?? null,
                        'building_id' => $room->building_id ?? null,
                        'error' => $ex->getMessage(),
                    ]);
            ...
            // Group rooms by building_id for easier processing in Flutter
            $roomsByBuilding = $publishedRooms->groupBy('building_id')->map(function ($buildingRooms) {
                return $buildingRooms->values(); // Reset array keys
            });

            return response()->json([
                'success' => true,
                'total_rooms' => $publishedRooms->count(),
            ...
            \Log::error('Error in getAllRoomsBatch: ' . $e->getMessage());
            \Log::error('Stack trace: ' . $e->getTraceAsString());
            
            return response()->json([
                'success' => false,
                'error' => 'Failed to fetch rooms data: ' . $e->getMessage(),
