    ...
    public function index(Request $request): JsonResponse
    {
        ...
        if ($user) {
            if ($user->role === 'super_admin') {
                ...
                // 1. Their own actions
                // 2. System events (no user_id)
                // 3. Published actions (public information)
                $query->where(function($q) use ($user) {
                    $q->where('user_id', $user->id) // Their own actions
                      ->orWhereNull('user_id') // System events
                      ->orWhereIn('action', [ // Published actions (public info)
                          'published', 
                          'published_building', 
                          'published_map', 
                          'published_deletion', 
            ...
            // Unauthenticated users see no logs
            $query->whereRaw('1 = 0');
        }
