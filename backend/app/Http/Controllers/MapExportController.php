...
class MapExportController extends Controller
{
    ...
    public function exportMap($id)
    {
        ...
        try {
            ...
            if (!mkdir($imagesDir, 0755, true)) {
                throw new \Exception('Failed to create images directory: ' . $imagesDir);
            }
            
            Log::info('MapExportController: Created temp directories', [
                'temp_dir' => $tempDir,
            ...
            if ($jsonContent === false) {
                ...
                throw new \Exception('Failed to encode export data to JSON: ' . $jsonError);
            }
            file_put_contents($tempDir . '/layout.json', $jsonContent);

            // Ensure all files are flushed to disk before zipping
            clearstatcache();

            // --- Generate ZIP archive robustly ---
            $zipFilename = 'map_export_' . preg_replace('/[^a-zA-Z0-9_-]/', '_', $map->name) . '_' . now()->format('Y-m-d_H-i-s') . '.zip';
            $zipPath = storage_path('app/temp/' . $zipFilename);

            // Clean up old temp zip if exists
            ...
            Log::info('Adding files to ZIP: ' . $zipPath);
            ...
            if (!file_exists($zipPath) || filesize($zipPath) === 0) {
                throw new \Exception('Final ZIP file missing or empty.');
            }
            Log::info('Successfully created ZIP: ' . $zipPath . ' (Size: ' . filesize($zipPath) . ')');

            // Clean temp directory before sending the file
            $this->deleteDirectory($tempDir);

            // Disable output buffering (prevents corrupt output)
            ...
            // Stream the file for download, with essential headers only
            ...
            // Clean up final ZIP file if it exists
            if (isset($zipPath) && file_exists($zipPath)) {
                unlink($zipPath);
            }
    ...
    public function importMap(Request $request)
    {
        try {
            ...
            // Allow up to ~300MB (value is in kilobytes). Adjust if you expect larger files.
            $request->validate([
                'map_file' => 'required|file|mimes:zip|max:307200'
            ]);
                    ...
                    if (isset($buildingData['employees']) && is_array($buildingData['employees'])) {
                        ...
                        foreach ($buildingData['employees'] as $empIndex => $employeeData) {
                            ...
                            if (\Illuminate\Support\Facades\Schema::hasColumn('employees', 'email')) {
                                // Generate unique email if empty to avoid constraint violation
                                if (empty($employeeData['email'])) {
                                    $employee->email = 'imported_' . time() . '_' . rand(1000, 9999) . '@imported.local';
                                } else {
    ...
    /**
     * Assemble file chunks into the final ZIP and trigger import.
     */
    public function queueImport(Request $request) {
        ...
        natsort($chunks); // Ensure order: 0, 1, 2, ...
        $out = fopen($assembledPath, 'w');
        foreach($chunks as $chunkFile) {
            if ($chunkFile === $assembledPath) continue;
        ...
        // Fake an UploadedFile object from this file for Laravel validation/logic
        $fakeFile = new \Illuminate\Http\UploadedFile(
            $assembledPath,
            $fileName,
            'application/zip',
            null,
            true // test mode, disables is_uploaded_file check
        );
        $importRequest = new Request([
            'map_file' => $fakeFile
        ], [], [], [], ['map_file' => $fakeFile]);
        $importRequest->setMethod('POST');
        $importRequest->files->set('map_file', $fakeFile);

        // Now call your importMap method
        $importResponse = $this->importMap($importRequest);
        // Optionally: Delete tmp_uploads dir after import
    ...
    /**
     * Handle a chunked file upload (receives individual chunk, robust version)
     */
    public function uploadChunk(Request $request)
    {
        try {
            $chunk = $request->file('chunk') ?? $request->file('file');
            $uploadId = $request->input('uploadId');
            $chunkIndex = (int) $request->input('chunkIndex');
            $totalChunks = (int) $request->input('totalChunks');
            
            if (!$chunk) {
                return response()->json(['success' => false, 'message' => 'Missing file data. Make sure the field is named chunk or file.'], 400);
            }
            if (!$uploadId) {
                return response()->json(['success' => false, 'message' => 'Missing uploadId in upload request.'], 400);
            }
            if ($chunkIndex === null || $chunkIndex === '') {
                return response()->json(['success' => false, 'message' => 'Missing chunkIndex in upload request.'], 400);
            }
            $tempDir = storage_path('app/tmp_uploads/' . $uploadId);
            if (!file_exists($tempDir)) {
                mkdir($tempDir, 0777, true);
            }
            $chunk->move($tempDir, $chunkIndex);

            $isComplete = ($totalChunks && $chunkIndex === $totalChunks - 1);

            // Minimal API: on last chunk, include 'complete' flag
            return response()->json([
                'success' => true,
            ...
            \Log::error('uploadChunk error', [
                'request' => $request->all(),
                'files' => $request->allFiles(),
                'error' => $e->getMessage()
            ]);
            return response()->json(['success' => false, 'message' => 'Upload chunk failed: ' . $e->getMessage()], 500);
        }
    }
    ...
    private function addDirectoryToZip($zip, $dir, $zipPath)
    {
        ...
        foreach ($files as $file) {
            ...
            if (is_dir($filePath)) {
                ...
                // Ensure file exists and is readable before adding to ZIP
                if (file_exists($filePath) && is_readable($filePath)) {
                    $result = $zip->addFile($filePath, $zipFilePath);
                    if (!$result) {
                        ...
                        Log::info("Successfully added file to ZIP", [
                            'file_path' => $filePath,
                            'zip_path' => $zipFilePath,
                    ...
                    Log::warning("File not found or not readable", [
                        'file_path' => $filePath
                    ]);
                }
            }
