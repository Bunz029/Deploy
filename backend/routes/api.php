...
use App\Http\Controllers\SuperAdmin\AdminSecurityController;
use App\Http\Controllers\SuperAdmin\AdminSessionController;
use App\Http\Controllers\ActivityLogExportController;
use App\Http\Controllers\SuperAdmin\AdminInviteController;
...
Route::prefix('published')->group(function () {
    ...
    Route::get('/buildings/all', [BuildingController::class, 'getAllPublished']); // New endpoint for all buildings without pagination
    Route::get('/buildings/metadata', [BuildingController::class, 'getPublishedMetadata']);
    ...
    Route::get('/rooms/all', [RoomController::class, 'getAllRoomsBatch']); // New batch endpoint
...
// Publication Routes
Route::middleware('auth:sanctum')->prefix('publish')->group(function () {
...
Route::prefix('auth')->group(function () {
    ...
    // Password reset
    Route::post('/password/forgot', [\App\Http\Controllers\Auth\PasswordResetController::class, 'request']);
    Route::get('/password/reset/{token}', [\App\Http\Controllers\Auth\PasswordResetController::class, 'show']);
...
// Activity Log Routes (requires authentication)
Route::middleware('auth:sanctum')->prefix('activity-logs')->group(function () {
    ...
    Route::get('/export', [ActivityLogExportController::class, 'export']);
});

// Map Export/Import Routes
Route::prefix('map-export')->group(function () {
    // Define static paths BEFORE parameter routes to avoid collisions
    Route::get('/test-zip', [MapExportController::class, 'testZip']); // Test ZIP creation
    Route::post('/upload-chunk', [MapExportController::class, 'uploadChunk']); // Chunked upload
    Route::post('/import', [MapExportController::class, 'queueImport']); // Async import
    Route::get('/import/status', [MapExportController::class, 'importStatus']); // Status check
    Route::post('/import/step', [MapExportController::class, 'importStep']); // Step-driven processing
    Route::post('/import/sync', [MapExportController::class, 'importMap']); // Legacy sync import

    // Constrain ID to numbers so "upload-chunk" doesn't match this route
    Route::get('/{id}', [MapExportController::class, 'exportMap'])->whereNumber('id');
});

// Superadmin Admin-Management Routes (skeleton)
Route::middleware(['auth:sanctum', 'superadmin'])->prefix('superadmin')->group(function () {
    ...
    Route::post('/admins/{admin}/reset-password', [AdminSecurityController::class, 'resetPassword']);
    Route::post('/admins/{admin}/reset-2fa', [AdminSecurityController::class, 'reset2FA']);
    Route::get('/admins/{admin}/sessions', [AdminSessionController::class, 'index']);
    Route::delete('/admins/{admin}/sessions/{tokenId}', [AdminSessionController::class, 'revoke']);
    Route::delete('/admins/{admin}/sessions', [AdminSessionController::class, 'revokeAll']);
    Route::post('/invites', [AdminInviteController::class, 'send']);
});

// Invite acceptance (public)
Route::get('/auth/invite/{token}', [AdminInviteController::class, 'validateToken']);
Route::post('/auth/accept-invite', [AdminInviteController::class, 'accept']);